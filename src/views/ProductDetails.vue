<template>
  <div class="mt-4">
    <router-link to="/products" class="text-lime-500 hover:text-indigo-500 font-semibold">Wstecz</router-link>
  </div>
  
  <div v-if="product" class="max-w-3xl mx-auto">
    <div class="flex justify-center">
      <h1 class="text-4xl font-bold text-center py-6">{{ id }}</h1>
    </div>
    <div class="flex justify-center mb-6">
      <div class="rounded-lg overflow-hidden shadow-md">
        <img :src="product.imageURL" alt="Product Image" class="object-cover w-48 h-48">
      </div>
    </div>
    <div class="mt-6">
      <table class="mx-auto border border-gray-300">
        ...
        <tr>
          ...
          <td class="px-4 py-2 border border-gray-300">{{ product.protein }}</td>
        </tr>
      </table>
    </div>
  </div>
